...
echo "======================================"
echo "Deploying canhnv.com ClusterIssuer to all clusters"
echo "======================================"
echo

# Check if secret is configured
if ! [ -f "$BASE_DIR/secrets/canhnv-com-secret.local.yaml" ]; then
    echo "ERROR: canhnv-com-secret.local.yaml not found!"
    echo "Please ensure $BASE_DIR/secrets/canhnv-com-secret.local.yaml exists with your Cloudflare API token"
    exit 1
fi
...
for cluster in "${CLUSTERS[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        echo "SUCCESS: canhnv.com ClusterIssuer deployed to $cluster"
    else
...
if [ ${#FAILED_CLUSTERS[@]} -eq 0 ]; then
    echo "SUCCESS: canhnv.com ClusterIssuer deployed to all clusters!"
else
...
echo "  kubectl describe clusterissuer canhnv-com-prod"
