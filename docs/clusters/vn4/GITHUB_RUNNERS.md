# VN4 Self-Hosted GitHub Runners
...
The VN4 infrastructure consists of 3 H2Cloud Vietnam VPS servers dedicated to GitHub Actions self-hosted runners. Each server runs 2 runner instances, one for each GitHub organization.
...
```
┌─────────────────────────────────────────────────────────────────┐
│                  VN4 Self-Hosted Runner Fleet                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  vps6 (160.191.245.246)                                  │    │
│  │  ├── actions.runner.murror-vn4-murror-1                 │    │
│  │  └── actions.runner.canh-nv-vn4-canh-nv-1               │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  vps13 (160.191.245.244)                                 │    │
│  │  ├── actions.runner.murror-vn4-murror-2                 │    │
│  │  └── actions.runner.canh-nv-vn4-canh-nv-2               │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  vps16 (160.191.245.234)                                 │    │
│  │  ├── actions.runner.murror-vn4-murror-3                 │    │
│  │  └── actions.runner.canh-nv-vn4-canh-nv-3               │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
|------|-------------|---------|--------|
| vps6 | 160.191.245.246 | vn4-murror-1, vn4-canh-nv-1 | self-hosted,linux,x64,vn4 |
| vps13 | 160.191.245.244 | vn4-murror-2, vn4-canh-nv-2 | self-hosted,linux,x64,vn4 |
| vps16 | 160.191.245.234 | vn4-murror-3, vn4-canh-nv-3 | self-hosted,linux,x64,vn4 |
...
PATs are stored locally in `vn4-runners.secrets` (gitignored):
...
- **Resources**: Minimum 2 CPU, 4GB RAM per node (VN4 has 4 CPU, 9.7GB RAM)
...
The inventory file `inventory.vn4-runners.yml` defines:
...
```bash
ssh root@160.191.245.246 "systemctl status actions.runner.murror-vn4-murror-1"
```
...
| vn4 | VN4 runner pool | Pool selection |
...
ssh root@160.191.245.246 "systemctl stop actions.runner.murror-vn4-murror-1"

# Start a runner
ssh root@160.191.245.246 "systemctl start actions.runner.murror-vn4-murror-1"
```
...
# Last 100 lines
ssh root@160.191.245.246 "journalctl -u actions.runner.murror-vn4-murror-1 -n 100"

# Logs since specific time
ssh root@160.191.245.246 "journalctl -u actions.runner.murror-vn4-murror-1 --since '1 hour ago'"
```
...
```bash
# On each node
ssh root@160.191.245.246 "apt update && apt upgrade -y"
```
...
1. Add host to `inventory.vn4-runners.yml`:
   ```yaml
   vps17:
     ansible_host: x.x.x.x
     ansible_user: root
     ansible_ssh_pass: "password"
     runner_name_suffix: "4"
   ```
...
2. Check network connectivity:
   ```bash
   ssh root@160.191.245.246 "curl -I https://github.com"
   ```
...
ssh root@160.191.245.246 "usermod -aG docker runner && systemctl restart actions.runner.murror-vn4-murror-1"
```
...
**Diagnosis**:
```bash
ssh root@160.191.245.246 "docker run --rm alpine ping -c 3 8.8.8.8"
```
...
**Diagnosis**:
```bash
ssh root@160.191.245.246 "journalctl -u actions.runner.murror-vn4-murror-1 -n 50"
```
...
ssh root@160.191.245.246 "rm -rf /home/runner/work/_temp/*"
```
...
- Store PATs in gitignored files (`vn4-runners.secrets`)
...
- [VN4 Cluster Overview](./README.md)
...
| Date | Change | Author |
|------|--------|--------|
| 2026-01-25 | Initial documentation for VN4 self-hosted runners | - |
