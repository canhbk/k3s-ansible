...
- **Scheduling**: Soft affinity for `longhorn: "true"` nodes, tolerates `dedicated=storage:NoSchedule`
- **Storage**: 20GB on `longhorn-retain`
...
| Endpoint | URL |
|----------|-----|
| HTTP API | `https://clickhouse.vn.k3s.canhnv.com` |
| Play UI  | `https://clickhouse.vn.k3s.canhnv.com/play` |
...
```bash
kubectl --context vn port-forward -n clickhouse svc/clickhouse 8123:8123
kubectl --context vn port-forward -n clickhouse svc/clickhouse 9000:9000
```
...
```bash
curl 'https://clickhouse.vn.k3s.canhnv.com/?user=admin&password=<pw>' \
...
kubectl --context vn exec -n clickhouse clickhouse-0 -- \
...
```bash
# Pod status
kubectl --context vn get pods -n clickhouse
...
```bash
kubectl --context vn describe pod -n clickhouse clickhouse-0
kubectl --context vn logs -n clickhouse clickhouse-0
```
