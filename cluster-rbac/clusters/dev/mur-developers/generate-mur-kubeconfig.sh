...
echo ""
echo "🔍 Verifying namespace listing..."
KUBECONFIG="$OUTPUT_FILE" kubectl auth can-i list namespaces && echo "✅ Can list namespaces: OK" || echo "❌ Cannot list namespaces"
echo ""
echo "📋 Accessible namespaces:"
KUBECONFIG="$OUTPUT_FILE" kubectl get ns -o name 2>/dev/null | grep -E "(mu-|nsp-alpha-murror)" || echo "⚠️  Unable to list namespaces"